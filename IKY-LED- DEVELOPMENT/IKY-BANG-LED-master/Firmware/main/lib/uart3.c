...
void USART3_Init(uint32_t pclk1,uint32_t baudrate)
{
		...
		//UART3 Pin
		GPIOB->CRH 	 &= ~(GPIO_CRH_MODE10 | GPIO_CRH_CNF10 | GPIO_CRH_MODE11 | GPIO_CRH_CNF11);  					// Clear PB11, PB10               
		GPIOB->CRH   |=   GPIO_CRH_CNF10_1 | GPIO_CRH_MODE10_0 | GPIO_CRH_MODE10_1; // USART3 Tx (PB10)   Alternate function output Open-drain
		GPIOB->CRH   |=  (GPIO_CRH_CNF11_0);                      // USART3 Rx (PB11) input floating
...
//			RINGBUF_Put(&USART3_RxRingBuff,c);
			RINGBUF_Put(&rfParser_RxRingBuff,c);
	}
