...
#define RF_RECV_APPLOG(...)  		NRF_LOG_PRINTF(__VA_ARGS__)
...
//		RF_RECV_APPLOG("RF_RECEIVE_READ_FIFO\r\n");
	...
	case RF_RECEIVE_PARSER:
		RF_RECV_APPLOG("rfPacketLength %d\r\n",rfPacketLength);
		for(i=1;i<rfPacketLength;i++)
		{
			RINGBUF_Put(&rfParser_RxRingBuff,rfRxBuffer[i]);
			RF_RECV_APPLOG("%02x",rfRxBuffer[i]);
		}
		RF_RECV_APPLOG("\r\n");
		RF_Receive_Phase = RF_RECEIVE_FINISH;
		break;
