...
void frameFill(void)
{
	...
	for(i=0;i<64;i++) 
	{
      framebuffer_displaybuffer[i] = 0x00;
      framebuffer_writebuffer[i] = 0x00;
	}
	__enable_irq();   
}
...
void ScrollMsgTask(void)
{
	...
	{		
			screen.top_pixel_len = GFXGetStringWidth((const char*)screen.top_msg);
			...
			if(++scroll == (screen.top_pixel_len + GFX_SCREEN_WIDTH))
			{
						scroll = 0;
			}				
